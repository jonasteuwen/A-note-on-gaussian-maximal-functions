...
\begin{equation*}
  \rho_t(s) := \frac{\e^{-|s|^2/4t}}{\pi^{\frac{d}2}} \frac1{(4t)^{\frac{d}2}}
\end{equation*}
\textbf{(verify)}
...
\textcite{Pineda2008} use a different minimal function $\widetilde{m}$,
namely:
\begin{equation*}
  \widetilde{m}(x) = \min\biggl\{\frac12, \frac1{|x|}\biggr\},
\end{equation*}
and their proof does not easily extend the range of $t$ up to $1$.
Using our notation they have shown that 
...
\begin{equation*}
  \sup_{(y, t) \in \widetilde{\Gamma}_x} |\e^{-t^2 \Delta} u(y)|
  \lesssim \sup_{r > 0}  \dashint_{B_r(x)} |u| \D\gamma.
\end{equation*}


Our contribution is the extension of the result \textcite[Lemma
2.1]{Pineda2008} to the complete Gaussian cone 
We define:
\begin{equation}
...
mean $\Gamma_x^{(1, a)}$. Using the following $m$-function:
...
\begin{lemma}\label{lem:Time-part-Mehler-time-transform}
  Let $t > 0$ and $\alpha \geq 2$. Then,
  \begin{equation}
    \label{eq:Time-part-Mehler-time-transform-1}
    \alpha \e^{\frac{t^2}{\alpha}} \e^{-t^2} \leq \frac{1 -
      \e^{-t^2}}{1 - \e^{-\frac{t^2}{\alpha}}} \leq \alpha,
  \end{equation}
  and
  \begin{equation}
...
\begin{proof}
  ...
  \begin{equation*}
    \alpha \exp\biggl(-t^2 \frac{\alpha - 1}{\alpha} \biggr) \leq
    \frac{1 - \e^{-t^2}}{1 - \e^{-\frac{t^2}{\alpha}}} \leq \alpha, 
  \end{equation*}
  ...
  Recalling that $\alpha > 2$ one can directly verify that the
  function
  \begin{equation*}
...
\begin{lemma}\label{lem:Exponential-estimates}
  ...
  \begin{equation}
    ...
    \exp \biggl (-\e^{-2\frac{t^2}\alpha} \frac{|x - y|^2}{1 - \e^{-2\frac{t^2}\alpha}}
    \biggr ) \leq \exp \biggl(-\frac{\alpha}{2\e^{2T^2}} \frac{|x -
      y|^2}{1 - \e^{-t^2}} \biggr).
  \end{equation}
\end{lemma}
\begin{proof}
...
\begin{align*}
  ...
  &\leq \exp \biggl(-\frac{1}{2\e^{\frac{T^2}{\alpha}}}
  \frac{\alpha}{\e^{T^2}} \frac{|x - y|^2}{1 - \e^{-t^2}} \biggr)\\
  &\leq \exp \biggl(-\frac{\alpha}{2\e^{2T^2}} \frac{|x - y|^2}{1 - \e^{-t^2}} \biggr).
\end{align*}
...
\begin{lemma}\label{lem:Kernel-estimates-1}
 Let $T > 0$, $\alpha \geq 4 \e^{2 T^2}$, $t \in (0, T]$ and $x, y \in
 \R^d$. Then:
  \begin{equation}
    \label{eq:Kernel-lemma-1-estimate} 
    M_{\frac{t^2}{\alpha}}(x, y) \leq \alpha^{\frac{d}2}
    \e^{\frac{t^2}4 |\la x, y \ra|} \exp\biggl (-\frac{\alpha}{4
      \e^{2 T^2}} \frac{|x - y|^2}{1 - \e^{-t^2}} \biggr ) M_{t^2}(x,
    y).
  \end{equation}
\end{lemma}
\begin{proof}
  ...
  \begin{align*}
    ...
    \alpha^{\frac{d}2} \e^{\frac{t^2}4 |\la x, y \ra|} \exp\biggl(\e^{-2t^2} \dfrac{|x - y|^2}{1 -
      \e^{-2t^2}} \biggr) \exp\biggl(-\e^{-2\frac{t^2}{\alpha}} \dfrac{|x - y|^2}{1
      - \e^{-2\frac{t^2}{\alpha}}}  \biggr)\\
    &\leq
    \alpha^{\frac{d}2} \e^{\frac{t^2}4 |\la x, y \ra|}
    \exp\biggl(\dfrac{|x - y|^2}{1 - \e^{-2t^2}} \biggr)
    \exp\biggl(-\e^{-2\frac{t^2}{\alpha}} \dfrac{|x - y|^2}{1 -
      \e^{-2\frac{t^2}{\alpha}}}  \biggr)\\ 
    &\leq \alpha^{\frac{d}2} \e^{\frac{t^2}4 |\la x, y \ra|} \exp
    \biggl (\biggl[1 -\frac{\alpha}{4\e^{2 T^2}} \biggr]
    \frac{|x - y|^2}{1 - \e^{-t^2}} \biggr ) \exp \biggl
    (-\frac{\alpha}{4\e^{2 T^2}} \frac{|x - y|^2}{1 - \e^{-t^2}} \biggr
    ).
  \end{align*}
  Finally, we apply the assumption $\alpha \geq 4 \e^{2 T^2}$ to obtain:
  \begin{equation*}
    \frac{M_{\frac{t^2}{\alpha}}(x, y)}{M_{t^2}(x, y)} \leq \alpha^{\frac{d}2}
    \e^{\frac{t^2}4 |\la x, y \ra|} \exp\biggl(-\frac{\alpha}{4\e^{2T^2}} \frac{|x -
      y|^2}{1 - \e^{-t^2}} \biggr).
  \end{equation*}
  Which is as asserted.
\end{proof}
...
\begin{proof}
  ...
  \begin{equation}
    ...
    M_{t^2}(y, \xi) \leq \e^{-\beta} \e^{-\alpha(1 + \alpha)} \frac{\e^{|y|^2}}{(1 - \e^{-t^2})^{\frac{d}2}}
     \exp\bigl(2^{k + 1} \alpha (1 + \alpha) \bigr) \e^{\beta 2^{k + 1}} \e^{-\beta 4^k},
  \end{equation}
...
is maximal for $\alpha = \frac12$ and after this value, $\beta$ is decreasing.
  ...
  \begin{equation}
    ...
    M_{t^2}(y, \xi) \lesssim_\alpha  \e^{-(\alpha + \beta)} \e^{\alpha^4} \e^{\alpha^2} \frac{ \e^{|x|^2} }{(1 - \e^{-t^2})^{\frac{d}2}} \e^{(\lambda + \beta) 2^{k + 1}} \e^{-\beta 4^k}.
  \end{equation}
...
Setting $\beta = \frac{A^2}{2 \e^{2T^2}}$.
  ...
  \begin{align*}
    ...
    &\hintedrel[rel1]{\leq} (M_\gamma u)(x) C_d A^d 2^{d(k + 1)} t^d \e^{-(t - |x|)^2}.
  \end{align*}
