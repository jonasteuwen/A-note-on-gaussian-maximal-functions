...
\begin{proof}
  ...
  \begin{align*}
    ...
    \biggr )\\
    &\leq \exp \biggl (\frac2{t^2}\biggl[\frac{1}{1 +
    ...
    \biggr] t^2 |\la x, y \ra| \biggr).
  \end{align*}
l{eq:Time-part-Mehler-time-transform-2}
  Using Cauchy-Schwarz and $2|x||y| \leq |x|^2 + |y|^2$ we get:
  \begin{align*}
    \exp \biggl (-\frac12 \frac{|x + y|^2}{1 + \e^{-\frac{t^2}{\alpha}}}
...
\begin{lemma}\label{lem:Gaussian-ball-shift-lemma}
  ...
  \begin{equation}\label{eq:Gaussian-ball-shift-lemma}
    \gamma(B_t(x)) \leq S_d \pi^{-\frac{d}2} \e^{-|x|^2} \e^{2 t |x|} \e^{-t^2} t^d.
  \end{equation}
\end{lemma}
\begin{proof}
  ...
  the notation, let $S_d$ be the surface area of the $d$-dimensional
  sphere. Using polar coordinates we then obtain:
  \begin{align*}
    \gamma(B_t(0)) &= \pi^{-\frac{d}2} \int_{B_t(0)} \e^{-|\xi|^2} \D\xi\\
    &= S_d \pi^{-\frac{d}2} \int_0^t \e^{-r^2} r^{d - 1} \D{r}\\
    &\leq S_d \pi^{-\frac{d}2} \e^{-t^2} t^d.
  \end{align*}
...
\begin{lemma}\label{lem:On-diagonal-kernel-estimates-on-Ck}
  ...
  \begin{equation*}
    M_{t^2}(y, \xi) \leq \frac{\e^{|y|^2} \e^{-A t |y|}}{(1 - \e^{-t^2})^{\frac{d}2}}
    \exp\bigl(2^{k + 1} A t |y| \bigr) \exp\biggl(-\frac{A^2}{2
      \e^{2t^2}} (2^k - 1)^2 \biggr).
  \end{equation*}
\end{lemma}
\begin{proof}
  Let $B = B_{At}(y)$ and let $C_k$ be as in
  \eqref{eq:C_k-annulus-decomposition}. For the sake of clarity we
  write $s := At$. Considering the first exponential which occurs in
  the Mehler kernel \eqref{eq:Mehler-kernel} together with
  \eqref{eq:C_k-annulus-decomposition-expand} gives for $k
  \geq 0$:
  \begin{align*}
  ...
  \begin{equation}
    ...
    |\langle y, \xi \rangle| \leq |\la y - \xi, \xi \ra| + |\la y, y \ra|
    \leq |y - \xi||y| + |y|^2.
  \end{equation}
  ...
  \end{equation*}
  So, using these we get for the last exponential in the Mehler kernel
  ...
  \begin{align*}
    ...
    \exp(|y - \xi||y|) \e^{|y|^2}\\
    &\hintedrel[rel1]{\leq} \exp(2^{k + 1} A t |y|) \e^{-A t |y|} \e^{|y|^2}.
  \end{align*}
  where (\hintref{rel1}) is due to $y$ and $\xi$ being in $C_k(B)$.

  Wrapping it up, we can estimate the Mehler kernel
  \eqref{eq:Mehler-kernel} $M_{t^2}$ on $C_k$ from above by:
  \begin{equation*}
    M_{t^2}(y, \xi) \leq \frac{\e^{|y|^2} \e^{-A t |y|}}{(1 - \e^{-t^2})^{\frac{d}2}}
    \exp\bigl(2^{k + 1} A t |y| \bigr) \exp\biggl(-\frac{A^2}{2 \e^{2t^2}} (2^k - 1)^2 \biggr).
  \end{equation*}
  Which is as claimed.
  \begin{equation*}
    M_{t^2}(y, \xi) \leq \frac{\e^{|y|^2} \e^{-A t |y|}}{(1 - \e^{-t^2})^{\frac{d}2}}
    \exp\bigl(2^{k + 1} A t |y| \bigr) \exp\biggl(-\frac{A^2}{2
      \e^{2t^2}} 4^k \biggr)  \exp\biggl(\frac{A^2}{\e^{2t^2}} 2^k \biggr)  \exp\biggl(-\frac{A^2}{2
      \e^{2t^2}} \biggr) 
  \end{equation*}
...
\begin{proof}
  ...
  \begin{equation}
    ...
    \gamma(B_t(x)) \leq S_d \pi^{-\frac{d}2} \e^{-|x|^2} \e^{2 t |x|} t^d.
  \end{equation}
  ...
  up the $C_k$. So,
  \begin{equation*}
    |x - \xi| \leq |x - y| + |\xi - y| \leq 2^{k + 1} At.
  \end{equation*}
  So, we can bound the integral on the right-hand side of
  \eqref{eq:Maximal-function-cone-intermediate-step-1} by
  \begin{align*}
    \int_{C_k}  |u| \,\D\gamma &\leq \gamma(B(x, 2^{k + 1} A t))
    \sup_{r > 0} \dashint_{B_r(x)} |u| \, \D\gamma\\
    &\hintedrel[rel1]{\leq} S_d \pi^{-\frac{d}2} \e^{2^{k + 2} A t |x|}
    2^{d(k + 1)} A^d t^d \e^{-|x|^2} \sup_{r > 0} \dashint_{B_r(x)}
    |u| \, \D\gamma\\
    &\leq S_d \pi^{-\frac{d}2} \e^{2^{k + 2} a A} 2^{d(k + 1)} A^d t^d
    \e^{-|x|^2} \sup_{r > 0} \dashint_{B_r(x)} |u| \, \D\gamma.
  \end{align*}
  Where (\hintref{rel1}) uses \autoref{lem:Gaussian-ball-shift-lemma}.
  The remark following \autoref{lem:Cone-Gaussians-comparable} gives
  us by assuming $|x - y| \leq aA m(x)$ the following estimate:
  \begin{equation*}
    \e^{|y|^2} \leq \e^{(1 + aA)^2 - 1} \e^{|x|^2}.
  \end{equation*}
  Let $G_{d,a,A} = S_d \pi^{-\frac{d}2}  \e^{(1 + aA)^2 - 1} A^d$, then
  \begin{equation*}
    \e^{|y|^2} \int_{C_k}  |u| \,\D\gamma \leq G_{d,a,A} \e^{2^{k + 2} a A}
    2^{d(k + 1)} t^d \sup_{r > 0} \dashint_{B_r(x)} |u| \, \D\gamma.
  \end{equation*}
  Using this, \eqref{eq:Mehler-kernel-estimate-one-sided-bound} and $t |y|
  \leq (1 + aA)$ we can bound the Mehler kernel \eqref{eq:Mehler-kernel} from
  above by:
  \begin{align*}
    M_{t^2}(y, \xi) \leq \frac{\e^{|y|^2}}{(1 - \e^{-t^2})^{\frac{d}2}}
    \exp\bigl(2^{k + 1} A (1 + aA) \bigr) \exp\biggl(-\frac{A^2}{2
      \e^{2t^2}} (2^k - 1)^2 \biggr).
  \end{align*}
  ...
  \begin{equation}
    ...
    \begin{split}
      ...
      \cdot) |u| \,\D\gamma\\
      &\leq  \frac{\e^{|y|^2}}{(1 - \e^{-t^2})^{\frac{d}2}} \sum_{k = 0}^\infty  \exp\biggl(-\frac{A^2}{2
      \e^{2a^2}} (2^k - 1)^2 \biggr) \exp\bigl(2^{k + 1} A (1 + aA)
    \bigr) \int_{C_k(B)}  |u| \,\D\gamma\\
      &\leq  G_{d,a,A} (\mathcal{M}_\gamma u)(y) \frac{t^d}{(1 - \e^{-t^2})^{\frac{d}2}} \sum_{k = 0}^\infty  \exp\biggl(-\frac{A^2}{2
      \e^{2a^2}} (2^k - 1)^2 \biggr) \exp\bigl(2^{k + 1} A (1 + aA)
    \bigr)  \e^{2^{k + 2} a A} 2^{d(k + 1)}\\
...
\begin{equation*}
  G_{d,a,A} \biggl(\frac{a^{2 d^{-1}}}{1 -
      \e^{-a^2}} \biggr)^{\frac{d}2} \sum_{k = 0}^\infty  \exp\biggl(-\frac{A^2}{2
      \e^{2a^2}} (2^k - 1)^2 \biggr) \exp\bigl(2^{k + 1} A (1 + aA)
    \bigr)  \e^{2^{k + 2} a A} 2^{d(k + 1)}\\
\end{equation*}
